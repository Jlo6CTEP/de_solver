...
class Window(QWidget):
    ...
    def create_main_window(self):
        ...
        self.input_initial_x.setFixedWidth(
            self.input_initial_x.sizeHint().width() / 5.0)
        self.input_initial_y.setFixedWidth(
            self.input_initial_y.sizeHint().width() / 5.0)
        ...
        self.plot_form.addRow(QLabel(c.ANALYTICAL_SOLUTION),
                              self.analytical[0])
    ...
    def create_aux_window(self):
        if not self.aux_window.isVisible():
            ...
            self.aux_list.setWordWrap(True)
            ...
            self.aux_window.setGeometry(
                self.geometry().x() + self.size().width() + 2.0,
                self.geometry().y(),
                self.size().width() / 3,
                self.size().height())
    ...
    def solve(self):
        ...
        try:
            return_message = self.canvas.plot_graph(
                self.input_de.text(), mode,
                float(self.input_final_x.text()),
                int(self.input_n_iter.text()),
                float(self.input_initial_x.text()),
                float(self.input_initial_y.text()))
